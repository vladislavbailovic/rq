...
mod test {
    ...
    #[test]
    fn comma_changes_strategy() {
        let mut parser = ExpressionParser::new("[0],[1]|keys");
        let result = parser.parse();
        assert!(result.is_ok(), "should not be an error");

        let filters = result.unwrap();
        assert_eq!(format!("{}", filters), "[0],[1]|keys");
    }
}
